...
`expo-calendar` module

## Installation

Install the package from `npm` registry:

`yarn add expo-calendar` or `npm install expo-calendar`
...
2.  Insert the following lines inside the dependencies block in `android/app/build.gradle`:
    ```gradle
    compile project(':expo-calendar')
    ```
3.  Add `new ExpoCalendarPackage()` to your module registry provider in `MainApplication.java`.
...
export default class App extends Component {
  ...
  doGreatJob = async () => {
    await ExpoCalendar.someGreatMethod();
  }

  render() {
    return (
      <View style={styles.container}>
        <Text style={styles.paragraph}>'expo-calendar' example</Text>
      </View>
    );
  }
}
...
### `ExpoCalendar.someGreatMethod(options)`
